...
record_run_probs <- combined %>% 
    filter(Team %in% c("Stranger Danger", COMPETITOR)) %>% 
    group_by(Team) %>% 
    summarise(record = mean(Outcome == "Win"), 
        runfrac = mean(Score / (Score + OpponentScore))) %>% 
    gather(Model, Rating, -Team) %>% 
    group_by(Model) %>% 
    arrange(Team) %>% 
